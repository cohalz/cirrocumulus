import { Construct, Fn, Aws } from "@aws-cdk/cdk"
import { IVpc, AmazonLinuxGeneration, InstanceType, SecurityGroup, CfnLaunchTemplate } from "@aws-cdk/aws-ec2"
import { EcsOptimizedAmi, Cluster } from "@aws-cdk/aws-ecs"
import { Role, PolicyStatement, CfnInstanceProfile } from "@aws-cdk/aws-iam"
import { AutoScalingGroup, CfnAutoScalingGroup, UpdateType } from "@aws-cdk/aws-autoscaling"
...
export class Ec2Cluster extends Construct {
  ...
  private createAutoScalingGroup = (scope: Construct, clusterName: string, vpc: IVpc, props: ClusterProps) => {
    ...
    if (this.spot) {
      ...
      if (props.instanceTypes.length > 1) throw new Error("When using on-demand instances, please set single instance type.")
    }

    const ami = new EcsOptimizedAmi({ generation: AmazonLinuxGeneration.AmazonLinux2 })
    ...
    const cfnInstanceProfile = asg.node.findChild("InstanceProfile") as CfnInstanceProfile

    const securityGroup = asg.node.findChild("InstanceSecurityGroup") as SecurityGroup

    this.instanceRole.addToPolicy(new PolicyStatement().addActions(
      "ec2:CreateTags",
    ...
    const launchTemplate = new CfnLaunchTemplate(scope, "AutoScalingGroupLaunchTemplate", {
      launchTemplateData: {
  ...
  private overrideAsg = (asg: AutoScalingGroup, launchTemplate: CfnLaunchTemplate, props: ClusterProps) => {
    const cfnAsg = asg.node.findChild("ASG") as CfnAutoScalingGroup
